...
twist  = [5.0, 5.0, 10.0] # Section twist [deg]
dih    = [0.0, 0.0]       # Segment dihedral [deg]

# Global Constraints & Targets
mirror           = True # Generate full wing from semi-wing
middle_gap       = 0    # Gap between semi-wings along the y-axis [m]
...
airfoils      = ['opt6.0', 'opt6.0', 'opt6.0', 'opt6.0'] # Airfoil per section (inner sections are defined by two airfoils: internal and external segments)
position      = [0, 0, 0]  # Root Leading Edge position (x, y, z)
tilt_angle    = [0.0, 0.0] # Mounting angles (Rx, Ry)
x_panels      = 15         # Chordwise panel discretization, the spanwise one is automatically defined inside flow5_element.

# Geometry Generation
winggeo = gm.trapez_wing(
...
# geo1: Dictionary for visualization
# elem1: Dictionary for Flow5 generation
geo1, elem1 = br.flow5_element(nome_elemento, type_elem, winggeo, airfoils, position, tilt_angle, x_panels)
...
geo2, elem1 = br.flow5_element('tail', 'ELEVATOR', tailgeo, ['Joukovsky 0009']*4, [1, 0, 0.0], [0.0, 0.0], 15)

# Lists for visualization and simulation
GeoElems = [geo1, geo2]

# --- Visualization ---
gm.plot_3d([geo1, geo2], airfoils_dir) #normal 3D plot