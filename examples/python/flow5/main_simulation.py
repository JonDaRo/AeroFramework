...
case_res = bf.flow5_case(flow5_dir, n_threads)
...
winggeo = wp.trapez_wing(True, 0, gov, span, taper, [0, 0], twist, [0, 0], 
                                  0.55, 3, 0.4, [0.15, 0.6], [-6, 6], [0, 10])

elem1, out1 = bf.flow5_element('wing', 'MAINWING', winggeo, 
                                  ['opt6.0']*4, 
                                  [0, 0, 0], [0, 0], 15)

# Tail
tailgeo = wp.trapez_wing(True, 0, gov, [10.0, 5.0], [10.0, 7.0], [10.0, 10.0], 
                                  [5.0, 0.0, 0.0], [10.0, 10.0], 0.1, 0.8, 0.38, 
                                  [0.1, 0.2], [-6, 6], [0, 40])

elem2, out2 = bf.flow5_element('tail', 'ELEVATOR', tailgeo, 
                                  ['Joukovsky 0009']*4, 
                                  [1, 0, 0], [0, 0], 15)
...
plane_name = 'plane1'
plane_res  = bf.flow5_plane(case_res, plane_name, mass_res, elements, airfoils_dir, polars_dir, 5)
...
an_name_1 = 'an1' # Analysis identifier
type_1 = 'T1'     # Analysis type
method = 'VLM2'   # Numerical method (e.g., Vortex Lattice Method 2)

anal_res_1 = bf.flow5_analysis(plane_res, an_name_1, type_1, method, FixTAS=speed_1)

## Analysis Setup (Analysis 2)
speed_2 = 25      # Cruise speed [m/s]
an_name_2 = 'an2' # Analysis identifier
type_2 = 'T5'     # Analysis type

anal_res_2 = bf.flow5_analysis(plane_res, an_name_2, type_2, method, FixTAS=speed_2)
...
run_res = bf.flow5_run(flow5_exe_path, [anal_res_1, anal_res_2], 
...
data_matrix1, name1 = bf.flow5_results(run_res[0], keys)
data_matrix2, name2 = bf.flow5_results(run_res[1], keys)
